...
public interface ContractRepository extends JpaRepository<ContractEntity, Long> {
    ...
    @Query(value = "select * from contracts left join customers  \n" +
            ...
            "AND (contracts.status = 4 or contracts.status = 5 or contracts.status = 6 or contracts.status = 7)  ORDER BY expected_start_date ASC ", nativeQuery = true)
    ...
    @Query(value = "  select * from contracts left join customers \n" +
            ...
            "            AND contracts.had_driver = CASE WHEN ?2 IS NULL THEN contracts.had_driver  ELSE ?2 END\n" +
            "            AND contracts.status = CASE WHEN ?3 IS NULL THEN contracts.status ELSE ?3 END " +
            "AND (contracts.status = 4 or contracts.status = 5 or contracts.status = 6 or contracts.status = 7) ORDER BY expected_start_date ASC" +
            "            ", nativeQuery = true)
    ...
    @Query(value = "  select * from contracts left join customers   \n" +
            ...
            "            AND contracts.had_driver = CASE WHEN ?2 IS NULL THEN contracts.had_driver  ELSE ?2 END\n" +
            "            AND contracts.status = CASE WHEN ?3 IS NULL THEN contracts.status ELSE ?3 END " +
            "AND(contracts.status = 4 or contracts.status = 5 or contracts.status = 6 or contracts.status = 7)", nativeQuery = true)
...
/// List Request
    ...
    @Query(value = "select * from contracts left join customers  \n" +
            ...
            "            AND contracts.had_driver = CASE WHEN ?2 IS NULL THEN contracts.had_driver  ELSE ?2 END\n" +
            "            AND contracts.status = CASE WHEN ?3 IS NULL THEN contracts.status ELSE ?3 END AND " +
            "(contracts.status = 1 or contracts.status = 2 or contracts.status = 3) ORDER BY expected_start_date ASC ", nativeQuery = true)
    ...
    @Query(value = "  select * from contracts left join customers \n" +
            ...
            "            AND contracts.had_driver = CASE WHEN ?2 IS NULL THEN contracts.had_driver  ELSE ?2 END\n" +
            "            AND contracts.status = CASE WHEN ?3 IS NULL THEN contracts.status ELSE ?3 END " +
            "AND (contracts.status = 1 or contracts.status = 2 or contracts.status = 3) ORDER BY expected_start_date ASC ", nativeQuery = true)
